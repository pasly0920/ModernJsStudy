...
if 코드 블록이 실행되면 if문의 코드 블록을 위한 블록 레벨 스코프를 생성해야 한다. 이를 위해 선언적 환경 레코드를 갖는 렉시컬 환경을 새롭게 생성하여 기존의 전역 레시컬 환경을 교체한다.

if 문이 종료되면 if 문의 코드 블록이 실행되기 이전에 렉시컬 환경으로 되돌린다.
